...
Origin in calculus of variaitons and work of Euler and Lagrange.

From @jordan1999introduction

Comparison to Newton-Raphson method - use of derivatives to find approximate solutions.

>"The idea behind VI is to first posit a family of densities and then to find a member of that family which is close to the target density. Closeness is measured by Kullback–Leibler divergence." 
`r tufte::quote_footer('@blei2017variational')`

>"We emphasize that MCMC and variational inference are dif-ferent approaches to solving the same problem. MCMC algorithms sample a Markov chain; variational algorithms solve an optimization problem. MCMC algorithms approximate the posterior with samples from the chain; variational algorithms approximate the posterior with the result of the optimization."
`r tufte::quote_footer('@blei2017variational')`

Decomposes log marginal likelihood into the lower bound ($\mathcal{L}(q)$) and the _Kullbach-Leibler divergence_ ($KL(q||p)$.)
...
(i.e. $q(\theta)$ is the posterior distribution). However, in practice one restricts
the possible distributions for $q(\theta)$.

Tradeoff: want options to be restricted enough to be tractable, sufficiently broad and flexible as to offer a good approximation of the posterior distribution.

Restrict possible distribution to use a parametric distribution $q(\theta | \omega)$ governed by the parameters $\omega$. Then $\mathcal{L}(q)$ is a function of $\omega$ and one may treat this as an optimisation problem.
...
3. Uncertainty quantification: the consensus matrix is a measure of uncertainty. Note, however, that this is not the principled uncertainty quantification of a converged Bayesian model.
4. Allows one to use Bayesian clustering models (such as the DMA or some of the integrative methods), leveraging some of their strengths.
