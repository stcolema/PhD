...
internet use data; spatial proteomics [@crook2018bayesian]; and image
analysis, where clustering is used for image segmentation and quantization. My
area of interest is in clustering 'omics data to improve understanding of 
...
defining subtypes within given diseases. Defining subtypes based upon behaviour
at the subcellular level rather than on phenotypic patterns can enable more 
effective treatment. Examples where cluster analysis has been used to propose 
...
MLE-based inference tends to be very quick method but suffers from several problems. 
It is:
...
MCMC samplers are very powerful and have aymptotic guarantees for describing their 
target distribution. However, there are several problems. Ensuring that a sampler has:

1. reached its stationary distribution;
2. explored the entire support of the traget distribution; and 
3. converged to its expectation

is difficult [@robert2018accelerating]. The _Gibbs sampler_ 
[@geman1984stochastic], a common choice of sampler, and can struggle to achieve 
these three goals in practice (if not in theory) due to high correlation between
samples [@bishop2006pattern]. 
Integrating over nuisance parameters as in a _collapsed Gibbs sampler_
[@liu1994collapsed] can improve rate of convergence upon the expectation, but 
can reduce the exploration rate of the chain. There exists many more flavours of 
MCMC that attack these 3 problems in different ways [@robert2018accelerating]. 
Methods which attempt to improve the ability of the sampler to explore the 
support of the target distribution such as simulated annealing 
[@kirkpatrick1983optimization; @atchade2011towards], global-local moves 
[] or split-merge steps [@green2001modelling; @jain2004split]; all offer some degree of success, 
...
method more similar to EM. This proposes a family of densities, $q(\theta)$, 
...
* VI is prone to becoming stuck in local minima, thus several restarts are 
required (as with EM);
* VI does not offer theoretical guarantees (unlike MCMC) [@blei2017variational]; 
and
* deriving the correct equations to apply VI in a specific situation is non-trivial.

A common theme across all of these methods is that they can struggle to converge 
...
A _consensus matrix_ is an $(N \times N)$ matrix that stores, for each pair of items, the proportion of model runs in which two items are clustered together. The consensus matrix is obtained by taking the average over the coclustering matrices obtained for each model restart.

Let $B^{(s)}$ be the coclustering matrix associated with the $s^{th}$ chain.
Then, for $S$ model runs, let the consensus matrix, $M$, be defined:
  
\begin{align}
M = \frac{1}{S}\sum_{s=1}^S B^{(s)}
\end{align}
...
of Bayesian 
inferenece (e.g. use of a prior, inference of $K$ through use of a 
...
the asymptotic guarantees of MCMC sampling. 
...
2. The aymptotic guarantees of MCMC methods mean that, in comparison to EM, if one runs the algorithm for long enough, one is guaranteed to describe the full posterior distribution and not only a local maximum.
