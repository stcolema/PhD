...
| 
| **Method:** 
| $\mathbb{M} \leftarrow 0_{N \times N}$ \{initialise the consensus matrix.\}
...
|   $\mathbb{M} \leftarrow \mathbb{M} + B^{(s)}$ 
| end
| 
| $\mathbb{M} \leftarrow \frac{1}{S} \mathbb{M}$ \{normalise the consensus matrix.\}
...
I test seven different scenarios that change various parameters in this model. The components are defined by parameters $(\mu_{kp}, \sigma^2)$ for each cluster within each feature. Within any scenario there is a common vector of possible means for each feature:

\begin{align}
\mu = (\mu_1, \ldots, \mu_K)
\end{align}

The indices here do not necessarily associated with the corresponding cluster (i.e. $\mu_1$ does not necessarily define the cluster labelled 1). Instead the mean associated with each cluster is randomly sampled without replacement for each feature. Let $\mu$ be ordered by increasing magnitude, i.e. $\mu_i < \mu_j \forall i < j$. Then $\Delta \mu$ be the distance between two consecutive means, i.e. $\mu$ may be re-written:
