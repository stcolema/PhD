...
The simulation study has two goals:
...
I test seven different scenarios that change various parameters in this model. The components are defined by parameters $(\mu_{kp}, \sigma^2)$ for each cluster within each feature. Within a scenario there is a common vector of possible means for each feature:

\begin{align}
\mu = (\mu_1, \ldots, \mu_K)
\end{align}

The indices here do not necessarily associated with the corresponding cluster (i.e. $\mu_1$ does not necessarily define cluster 1). Instead the mean associated with each cluster is randomly sampled without replacement for each feature. Let $\mu$ be ordered by increasing magnitude, i.e. $\mu_i < \mu_j \forall i < j$. Then $\Delta \mu$ be the distance between two consecutive means, i.e. $\mu$ may be re-written:
...
As the mean of a cluster in a feature is sampled from $\mu$, this means that additional features imparts more information about the clustering structure. There is $K!$ possible unique permutations of $\mu$. This means that as the number of relevant features grows the clarity of the structure becomes improves.

The scenarios tested are:

1. The 2D case;
2. The lack-of-structure case in 2 dimensions;
3. The base case for which scenarios 4-6 are direct variations;
...
medicine one might believe that some of the clusters of patients present might
...
The samples from the chains that pass the Shapiro-Wilks test are then pooled for the comparison beyond methods. I do not drop the entire analysis based upon overall convergence as doing so might lead some scenarios to having a very small number of data points for the Bayesian inference in the model comparison plots. Pooling samples is done to avoid model selection problems and to enable comparison across simulations. The pooling will mean that the statistics for the Bayesian inference in model comparison will might be more robust to modes (as we might have several different modes present in the pooled samples), thus inflating performance. This will be an upper limit on the performance of the Bayesian method.
...
For a collapsed Gibbs sampler the number of features present influences how diverse a range of partitions the sampler can explore. One can see in figure [exploration] that for a small number features the sampler suggests a different partition with every sample, but as  the number of features scales the diversity becomes more and more limited. This is not inherently a flaw; as the number of features increases so too does the amount of information pertaining to the structure in the dataset. However, this does mean that it is more probable that the sampler becomes trapped in any modes it encounters as the attraction of any single mode becomes more extreme as too does the distance between modes. 
...
However, a question for the practitioner is, how does one choose $S$ and $R$, the parameters of the ensemble? This is not a problem with an analytic solution. In theory, and similarly to Bayesian inference, the solution is to let $S \to \infty, R \to \infty$. In practice this is not an answer. If the data is noisy, believed to contain many irrelevant features or poor distinction between sub-populations, one should try increase the value of $R$. As the total number of features scales so too should $S$, the number of learners used in the ensemble. I think comparing the consensus matrices for different values of $S$ and $R$ is also useful. Consider this method for deciding if $(R', S')$ are sufficiently large:

1. Generate $S'$ Markov chains of $R'$ partitions.
2. Use these to construct 3 consensus matrices. For some $a, b \in [0,1]$ (for example $a=b=\frac{3}{4}$), compure CMs for each of:
  <!-- * Consensus$(0.75 R', 0.75 S')$; -->
  * Consensus$(a R', S')$;
  * Consensus$(R', b S')$;
  * Consensus$(R', S')$;
3. If the consensus matrices are sufficiently stabilised, stop. Possibly a stopping rule could be created base upon some threshold $\epsilon \in \mathbb{R_{+}}$ (for example see equation \@ref(eq:stoppingRule)), though in practice a visual comparison of the CMs should be sufficient. If the matrices are not considered sufficiently stable for the increase in $R$, repeat for some $R'' > R'$; and similarly for the value used for $S$.
