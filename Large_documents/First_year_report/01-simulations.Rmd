...
Within any scenario there is a common vector of possible means for each feature:

\begin{align}
\mu = (\mu_1, \ldots, \mu_K)
\end{align}

The indices here do not necessarily associated with the corresponding cluster (i.e. $\mu_1$ does not necessarily define the cluster labelled 1). Instead the mean vector is randomly permuted for each feature before generating the clusters; thus $\nu$, the vector of means in the $p^{th}$ feature, has the same values as $\mu$ but reordered. It is the $k^{th}$ value of this permuted vector that defines the values generated for the $k^{th}$ component in the $p^{th}$ feature, not the $k^{th}$ value from $\mu$ (although $\mu$ is one of the possible permutations of itself). 
...
I test seven different scenarios that change various parameters in this model. The scenarios tested and there defining parameters are shown below in table \ref{scenario_table}. In this case I have separated the number of relevant signals ($P_s$), such that $P=P_s + P_n$.s
