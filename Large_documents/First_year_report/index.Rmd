...
R & & \textrm{The number of iterations for which a sampler is run in Bayesian or consensus inference.} \\
S & & \textrm{The number of samples generated for Bayesian or consensus inference.} \\
...
for Bayesian inference one of either stochastic (_sampling based_, such as 
Markov Chain Monte-Carlo (**MCMC**) methods) or deterministic
(_variational inference_, (**VI**)) methods tend to be used.
...
global-local moves [] or split-merge steps [@green2001modelling; @jain2004split];
...
Kullback–Leibler divergence  [@blei2017variational]. VI is intended as a fast alternative to MCMC when
...
each mixutre model would be producing samples that are better than random as the 
