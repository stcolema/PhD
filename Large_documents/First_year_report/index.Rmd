...
Mixutre models and their extensions are powerful cluster analysis tools, 
...
@gelman2013bayesian]. These methods can be extended to perform integrative 
clustering, sharing information across multiple paired datasets [see for 
example @kirk2012bayesian; @lock2013bayesian; @gabasova2017clusternomics], 
...
implementations have various advantages and disadvantages.
...
(**PSM**) and the consensus matrix. These are defined as follows:

```{definition, psm, name = "Posterior similarity matrix"}
A _posterior similarity matrix_ is an $(N \times N)$ matrix that stores, for 
each pair of items, the proportion of MCMC samples for which the pair are
clustered together. Thus the $(i, j)^{th}$ entry is the proportion of times
the $i^{th}$ and $j^{th}$ items are allocated to the same cluster. This means
that the PSM is a symmetric matrix with all entries between 0 and 1 and 
...
Similar to the PSM, a _consensus matrix_ is an $(N \times N)$ matrix that stores,
for each pair of items, the proportion of model runs in which the pair are 
clustered together. The consensus matrix is obtained by taking the average over
the coclustering matrices obtained for each model restart.

Let $B^{(i)}$ be the coclustering matrix associated with the $i^{th}$ chain.
Then, for $I$ chains, let the consensus matrix, $M$, be defined:
  
\begin{align}
M = \frac{1}{I}\sum_{i=1}^I B^{(i)}.
\end{align}

As with the PSM, the consensus matrix is a symmetric matrix with all entries 
between 0 and 1 and diagonal entries of 1.
