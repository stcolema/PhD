...
of $K_l$ distributions. This means that the full model density is the weighted 
sum of the probability density functions associated with each distribution where
the weights, $\pi_{lk}$, are the proportion of the total population assigned to the 
$kth$ cluster:

$$p(X_{li}| c_{li} = k) = \pi_{lk} f(X_{li} | \theta_{lk}),$$
$$p(X_{li}) = \sum_{k=1}^{K_l} \pi_{lk} f(X_{li} | \theta_{lk}).$$
