...
Let $m_k$ denote the $kth$ centroid. Then if $\mathbf{Z}$ is the $N \times K$ classification matrix, with 
...
If we define the $K \times K$ matrix $\mathbf{L}$ with $(k, k)th$ entries of $N_k$ and 0's elsewhere and the gram matrix $\mathbf{\Delta}$ as the matrix with $(i,j)th$ entries $\delta(x_i, x_j)$, then, according to @gonen2014localized, this can be rephrased as a trace maximisation problem:

\[
\underset{\mathbf{Z}}{\operatorname{argmax}} \textrm{ tr}(\mathbf{L^{\frac{1}{2}}Z^T\Delta Z L^{\frac{1}{2}} - \Delta})
\]

This is subject to the contraints:
...
The binary variables, $z_{ik}$, make this problem very difficult to solve [@gonen2014localized]. Relaxing this constraint and letting $\mathbf{ZL^{\frac{1}{2}}} = \mathbf{H}$ and letting $\mathbf{H}$ take arbitrary real values constrained by $\mathbf{H^TH}=\mathbf{I_k}$ makes the optimisation problem more feasible:

\[
\underset{\mathbf{H}}{\operatorname{argmax}} \textrm{ tr}(\mathbf{H^T\Delta H - \Delta}).
\]
